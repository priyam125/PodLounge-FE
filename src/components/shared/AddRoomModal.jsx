...
import { createRoom as createRoomApi} from "../../api/roomsApi";
...
const AddRoomModal = ({ showModal, setShowModal }) => {
  ...
  const createRoom = async() => {
    ...
    console.log("Creating room", topic, selectedRoomType);

    try {
      const response  = await createRoomApi({ topic, roomType: selectedRoomType });
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center rounded-[20px]">
      <div className="w-1/2 max-w-[500px] bg-[#1d1d1d] relative">
        ...
        <div className="p-8 border-b-[2px] border-b-[#262626]">
          ...
          <TextInput fullwidth={true} onChange={(e) => setTopic(e.target.value)} value={topic} />
          <h2 className="text-lg my-2.5 font-bold">Room types</h2>
          <div className="grid grid-cols-3 gap-6">
            <div
              onClick={() => setSelectedRoomType("Open")}
              className={`${
                selectedRoomType === "Open" ? "bg-[#262626]" : ""
              } flex flex-col items-center p-2.5 rounded-[10px] cursor-pointer`}
            >
              <img src="/images/Globe.png" alt="globe" className="" />
              <span>Open</span>
            </div>
            <div
              onClick={() => setSelectedRoomType("Social")}
              className={`${
                selectedRoomType === "Social" ? "bg-[#262626]" : ""
              } flex flex-col items-center p-2.5 rounded-[10px] cursor-pointer`}
            >
            ...
            <div
              ...
              className={`${
                selectedRoomType === "Private" ? "bg-[#262626]" : ""
              } flex flex-col items-center p-2.5 rounded-[10px] cursor-pointer`}
            >
        ...
        <div className="flex flex-col items-center gap-5 p-7 text-center">
          ...
          <button onClick={createRoom} className="flex items-center gap-2 border-none outline-none bg-[#20bd5f] text-white py-2 px-8 rounded-[20px] hover:bg-[#16733b] transition-all ease-in-out duration-300">
            <GoIcon />
            Let's Go
          </button>
        </div>
